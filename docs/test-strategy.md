...
| Python unit/API | Reproducible 1,000-row model corpus, scenario-grouped validation, dataset-aware model cache, shared JWT validation, role denial, deterministic training, 12-runbook RAG parsing/index versioning, citation binding, fixed 24-question retrieval quality, abstention, English/Polish/Ukrainian and common encoded prompt-injection rejection for questions and runbook ingestion, JSON Schema rejection, deterministic replay IDs, bounded producer retries |
...
Disposable Kubernetes browser and load gates:
...
13. Run all 24 supported questions in the fixed RAG evaluation, require at least 90% retrieval
    recall within the top three, require every unrelated question to abstain, reject all 22 direct
    prompt-safety attacks in both questions and runbook ingestion without rejecting the 11
    security-related legitimate questions, and exercise a cited answer through Nginx.
...
16. Render both Kubernetes overlays, run strict Kubernetes 1.36 schema checks, and execute
    the disposable kind acceptance test including browser journeys, bounded load thresholds,
    and PostgreSQL and Redpanda pod replacement.
17. Run CodeQL and Trivy source, secret, configuration, and image gates, then retain
    CycloneDX SBOMs for every runtime image.
